/*Write a java program that establishes a connection to mysql
  database successfully. If the connection is successful,
it should display a message “Connection Established successfully”.
 In case, it is not able to do so due to any exception,
it should display the message “Connection could  not be established “.
 If there is an exception, it should also display the description of
 the exception.*/
import java.util.*;
public class JDBC_Q1 {
    public static void main(String[] args) {
        System.out.println("hello");
    }
}
